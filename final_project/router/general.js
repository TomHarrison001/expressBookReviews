...
public_users.get("/isbn/:isbn", async function (req, res) {
  ...
  try {
    const response = await axios.get(`URL_TO_GET_BOOKS_BY_ISBN/${isbn}`);
...
public_users.get("/author/:author", async function (req, res) {
  ...
  try {
    ...
    res.status(500).json({
      message: "Error fetching books by this author",
      error: error.message,
    });
  }
});
...
public_users.get("/title/:title", async function (req, res) {
  ...
  try {
    ...
    res
      .status(500)
      .json({
        message: "Error fetching books by this title",
        error: error.message,
      });
  }
});
...
public_users.get("/review/:isbn", function (req, res) {
  ...
  if (filteredBooks.length > 0) {
    let review = filteredBooks[0].reviews;
    res.status(200).send(review);
  } else {
    res.status(404).json({ message: "Books with this ISBN not found" });
  }
});
